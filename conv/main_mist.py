...
logger = logging.getLogger("conv_mist")

pattern = os.getenv("MIST_PATTERN", "(\d{4}-\d{2}-\d{2})_(\d{2}-\d{2}-\d{2})")
MIST_RE = re.compile(pattern)
HEALTH_CONTAINER_MIST_LPI = os.getenv("HEALTH_CONTAINER_MIST_LPI", "health:container_conv_mist")

def main():
    setup_logging(process_name="conv_mist")
    ...
    start_heartbeat(redis_client=redis_db, key=HEALTH_CONTAINER_MIST_LPI)
